"""
File: service.py
Purpose: Main orchestration layer for the ASR stage.
"""
...
class ASRService:
    ...
    def transcribe(self, request: ASRRequest) -> ASRTranscriptResult:
        """Run the full ASR flow for one media source."""
        ...
        try:
            ...
            quality = build_quality_summary(
                ...
                duration_seconds=float(
                    transcription.get("audio_duration_seconds", 0.0) or 0.0
                ),
            )
            return ASRTranscriptResult(
                ...
                transcriber_backend=str(
                    transcription.get("transcriber_backend", "groq")
                ),
                transcriber_model=str(
                    transcription.get("transcriber_model", self.transcriber.model)
                ),
            )
        finally:
...
# Exposes the compact ASR service API used by the main pipeline.

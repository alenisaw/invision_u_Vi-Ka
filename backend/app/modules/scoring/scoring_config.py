...
Purpose: Typed loader for the YAML-based scoring config.
"""
...
def _load_yaml() -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"Scoring config file is missing: {config_path}") from exc
    except yaml.YAMLError as exc:  # pragma: no cover
        raise RuntimeError(f"Scoring config file is invalid YAML: {config_path}") from exc

    if not isinstance(payload, dict):
        raise RuntimeError(f"Scoring config file must contain a mapping object: {config_path}")
    return payload

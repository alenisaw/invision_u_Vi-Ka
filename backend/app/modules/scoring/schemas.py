"""
File: schemas.py
Purpose: Shared Pydantic models for the scoring stage.
"""
...
class SignalPayload(BaseModel):
    """Structured signal produced by the extraction pipeline."""
...
class SignalEnvelope(BaseModel):
    """Canonical extraction-to-scoring contract."""

    model_config = ConfigDict(extra="ignore", populate_by_name=True)

    candidate_id: UUID
    signal_schema_version: str = Field(..., min_length=1)
    extraction_model_version: str = Field(default="unknown", min_length=1)
...
class CandidateScore(BaseModel):
    """Final scoring output for one candidate."""

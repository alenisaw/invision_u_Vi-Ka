"""
File: ml_model.py
Purpose: Compact ML refinement layer for the scoring stage.
"""
...
from .scoring_config import PROGRAM_CATALOG
...
class HybridScoringModel:
    """Thin wrapper around the regression model used by the scoring stage."""
    ...
    def load(cls, path: str | Path) -> "HybridScoringModel":
        ...
        if metadata.get("artifact_type") != "scoring_model":
        ...
        if not isinstance(payload, dict) or "model" not in payload or "feature_names" not in payload:
            raise RuntimeError(f"invalid scoring model artifact payload: {artifact_path}")

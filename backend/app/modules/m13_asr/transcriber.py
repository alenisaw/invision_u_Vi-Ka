"""
File: transcriber.py
Purpose: API-backed Whisper Large V3 Turbo integration for M13.
"""
...
class GroqWhisperTranscriber:
    ...
    def transcribe(self, media_path: str | Path, language: str = "auto") -> dict[str, Any]:
        ...
        if not self.enabled:
            raise RuntimeError("M13 ASR API key is not set.")
    ...
    def _prepare_audio_input(self, media_path: Path) -> tuple[Path, tuple[Path, ...]]:
        ...
        if result.returncode != 0:
            raise RuntimeError("ffmpeg failed to extract audio for M13 transcription.")

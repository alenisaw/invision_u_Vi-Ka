...
Purpose: Groq-backed LLM client for M5 signal extraction (OpenAI-compatible API).
"""
...
from .gemini_client import SignalGroupSpec
...
class GroqSignalClient:
    """OpenAI-compatible Groq client for per-group M5 extraction."""
    ...
    def extract_group(
        ...
        raw_signals = self._normalize_signals(parsed, spec.name)
    ...
    def _parse_response(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(choices, list) or not choices:
            logger.error("Groq response has no choices. Payload: %s", json.dumps(payload, ensure_ascii=False)[:2000])
            raise ValueError("Groq response does not contain choices.")
        message = choices[0].get("message", {})
        content = message.get("content", "")
        if not content:
            logger.error("Groq response has empty content. Choice: %s", json.dumps(choices[0], ensure_ascii=False)[:2000])
    ...
    @staticmethod
    def _normalize_signals(parsed: dict[str, Any], group_name: str) -> dict[str, Any]:
        """Accept signals as dict or list-of-objects with signal_name key."""
        ...
        if isinstance(raw_signals, list):
            ...
            for item in raw_signals:
                if isinstance(item, dict) and "signal_name" in item:
                    name = item.pop("signal_name")
                    normalized[name] = item
            if normalized:
                logger.info("Groq returned signals as list for group %s, normalized %d items", group_name, len(normalized))
                return normalized
        logger.error("Groq response for group %s has unexpected 'signals' format: %s", group_name, type(raw_signals).__name__)
        raise ValueError("Groq response does not contain `signals`.")

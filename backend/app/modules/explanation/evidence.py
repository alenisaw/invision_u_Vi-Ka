"""
File: evidence.py
Purpose: Evidence selection helpers for explanation output.
"""

from __future__ import annotations

from .schemas import EvidenceItem, ExplanationInput, ExplanationSignalContext
...
def collect_factor_evidence(handoff: ExplanationInput, factor_name: str, limit: int = 2) -> list[EvidenceItem]:
...
def collect_factor_signal_contexts(
    ...
    factor_name: str,
    limit: int = 2,
) -> list[tuple[str, ExplanationSignalContext]]:
    """Collect the most relevant signal contexts for one factor."""

    contexts: list[tuple[str, ExplanationSignalContext]] = []
    preferred_signals = FACTOR_SIGNAL_MAP.get(factor_name, ())
    for signal_name in preferred_signals:
        signal = handoff.signal_context.get(signal_name)
        if signal is None:

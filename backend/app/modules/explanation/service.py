"""
File: service.py
Purpose: Reviewer-facing explanation formatting for the explanation stage.
"""
...
from ..scoring.schemas import CandidateScore, SignalEnvelope
...
from .schemas import ExplanationInput, ExplanationReport, FactorBlock


class ExplanationService:
    """Formats scoring outputs into an auditable explanation bundle."""
    ...
    def build_report(self, handoff: ExplanationInput) -> ExplanationReport:
        factor_blocks = [
            FactorBlock(
    ...
    async def generate(
        ...
        """Build and persist explanation output from the supplied scoring result."""

        handoff = self.scoring_service.build_explanation_input(envelope, score)
        report = self.build_report(handoff)
        await self._persist_report(candidate_id, report)
        return report

    async def _persist_report(self, candidate_id, report: ExplanationReport) -> None:
    ...
    def _build_summary(
        ...
        source_clause = (
            f" Основные подтверждения получены из источников: {', '.join(self._format_sources(evidence_sources[:2]))}."
            if evidence_sources
            else ""
        )
        if handoff.manual_review_required:
            return (
                ...
                "однако кейс требует ручной проверки, поскольку текущая доказательная база неполная или нестабильная."
                f"{source_clause}"
            )
        if caution_blocks:
            ...
            return (
                f"Кандидат демонстрирует сильные стороны по направлениям {strengths}, "
                f"однако перед финальным решением комиссии необходимо отдельно оценить сигналы риска: {caution_titles}."
                f"{source_clause}"
            )
        return (
            f"Кандидат демонстрирует сильные стороны по направлениям {strengths}, "
    ...
    def _build_reviewer_guidance(self, handoff: ExplanationInput, caution_blocks: list) -> str:
        if handoff.manual_review_required:
            return (
                "Проверьте согласованность доказательств, качество входных материалов и предупреждающие флаги "
                "перед финальным решением комиссии. Неполные данные следует трактовать как нехватку информации, "
                "а не как автоматическое подтверждение слабого потенциала кандидата."
            )
        if caution_blocks:
            return (
                "Используйте оценку как рабочий ориентир, но обязательно пройдитесь по предупреждениям "
        ...
        return (
            "Используйте explanation как опору для решения: итоговый вердикт остается за комиссией, "
            "но ключевые положительные факторы и предупреждения уже структурированы для обсуждения."
        )

    def _build_factor_summary(self, handoff: ExplanationInput, factor) -> str:

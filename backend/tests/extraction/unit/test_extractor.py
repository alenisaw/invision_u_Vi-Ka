...
from backend.app.modules.extraction.extractor import HeuristicSignalExtractor
from backend.app.modules.extraction.schemas import ExtractionRequest


class HeuristicSignalExtractorTests(unittest.TestCase):
    def build_request(self) -> ExtractionRequest:
    ...
    def test_extract_skips_empty_sources_cleanly(self) -> None:
        extractor = HeuristicSignalExtractor()
        signals = extractor.extract(ExtractionRequest(candidate_id=uuid4(), selected_program="Foundation Year"))
    ...
    def test_behavioral_cues_work_for_mixed_language_initiative(self) -> None:
        ...
        request = ExtractionRequest(
            ...
            video_transcript=(
                "РЇ СЃР°РјР° opened a school media club, found a teacher adviser, and ran weekly sessions where we recorded short interviews."
            ),
        )
